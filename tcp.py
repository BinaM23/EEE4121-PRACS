...
class TCPTopo(Topo):
    ...
    def build(self, n=2):
        ...
        bw_net=args.bw_net
        delay=args.delay  #can't plot ping if use 'ms' in arg
        maxq=args.maxq
...
def start_ping(net):
    ...
    #ping_train2=h1.popen("ping -c %s -i 0.1 %s > %s/ping.txt" % (args.time*10,h1.IP(), args.dir), shell=True)    
    ping_train.communicate() #comment out?
...
def timer(net,h1,h2):
    times=[]
    for i in range(3):
        my_string="curl -o index.html -s -w %%{time_total} %s/http/index.html" %(h1.IP())
...
def tcp():
    ...
    print("mean fetch time: %s" % mymean)
