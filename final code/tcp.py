...
def start_iperf(net):
    ...
    client=h2.popen("iperf -s -w 16m")  #second server
...
#function to fetch packet from server 3 times
#loop is done in the terminal and the times are written to a .txt file for later calculation
#this elimintates additional delay due to calling python for loops
def fecthpckt(net,h1,h2):
     my_string="for i in {1..3}; do curl -o index.html -s -w \%{time_total}'\n' 10.0.0.1/http/index.html >> ./times.txt; done" 
     h2.popen(my_string,shell=True) #,stdout=PIPE
     #duration=float(time.stdout.read()) #read back time form stdout
     #return duration

#function defined to calculate mean times and std deviation
def stats(): #times(net,h1,h2)
   arr=[]
   f=open("./times.txt","r")
   data=f.read().strip().split("\n") #.strip()
   f.close
   arr=[float(line.strip()) for line in data] #.strip())
   #for i in range(3):
   ...
   for t in arr:
        tot+=(t-mean)**2
   standard_dev=math.sqrt(tot/len(arr))
...
def tcp():
    ...
    qmon = start_qmon(iface='s0-eth2',
                      outfile='%s/q.txt' % (args.dir)) #changed to 1
    ...
    times=[] #store the times for calculation
    start_time = time()
    h1=net.get('h1')
    h2=net.get('h2')
    while True:
        # do the measurement (say) 3 times.
        #times=stats(net,h1,h2) #calls our timer
        fecthpckt(net,h1,h2) #for loop repeats on terminal rather than in python so no delay due to invoking python loop
        sleep(5)
        now = time()
    ...
    Popen("rm times.txt",shell=True).wait()
